...
function CustomScroll(el, options) {
  ...
  for (var option in options) {
    if (option === 'bindTo') {
      ...
      if (horizontal) {
        for (
          var _i = 0, horizontal_1 = horizontal;
          _i < horizontal_1.length;
          _i++
        ) {
...
CustomScroll.prototype = {
  ...
  start: function(ev) {
    if (_this.typeStarted && ev.type !== _this.typeStarted) return;
  ...
  translate: function(x, y) {
    _this.scrollerStyle.transform =
      'translate(' + x + 'px,' + y + 'px)' + _this.translateZ;
    _this.x = x;
    _this.y = y;
  },
  refresh: function() {
