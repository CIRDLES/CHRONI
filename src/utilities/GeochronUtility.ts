...
const CREDENTIALS_URL: string =
  'https://app.geosamples.org/webservices/credentials_service.php';
const BASE_ALIQUOT_URL: string =
  'http://www.geochronportal.org/getxml.php?igsn=';
...
export class GeochronUtility {
  ...
  public downloadIGSN(
    ...
    return new Observable(observer => {
      ...
      const tempPath =
        igsn + '_temp_' + new Date().getMilliseconds().toString();
  ...
  private validateAndTransferTempFile(file: FileEntry): Observable<boolean> {
    return new Observable(observer => {
      this.xmlUtil.convertXMLtoJSON(file, 'temp').subscribe(
        json => {
          let type = null;
          let fileName = null;
          ...
          if (!type || !fileName) {
            this.displayToast(
              'ERROR: the file specified is not a valid Aliquot or Report Settings XML file'
            );
            ...
            this.fileUtil.moveFile(file.name, newPath, true).subscribe(
              ...
              error => {
                this.displayToast(
                  'ERROR: ' +
                    fileName +
                    ' could not be downloaded to the ' +
                    type +
                    ' directory'
                );
                this.fileUtil.removeFile(file.name, 'temp');
  ...
  public downloadFromURL(url: string): Observable<boolean> {
    const split = url.split('/');
    const split2 = split[split.length - 1].split('.');
    const tempPath =
      split2[0] + '_temp_' + new Date().getMilliseconds().toString();
    return new Observable(observer => {
  ...
  public validateCredentials(
    ...
    let extractValid = (res: Response | any) => {
      ...
      if (match) {
        ...
        return (
          resultJSON && resultJSON['valid'] && resultJSON['valid'] === 'yes'
        );
      } else return false;
    };
    return new Observable(observer => {
      ...
      let data =
        ...
        '&submit=submit';
      this.http
        .post(CREDENTIALS_URL, data, options)
        .subscribe(
  ...
  public saveCurrentUser(username: string, password: string): Observable<any> {
    return new Observable(observer => {
      ...
      ob.subscribe(value => {
        numFinsished++;
        if (numFinsished >= 3) observer.complete();
      });
    });
  }
